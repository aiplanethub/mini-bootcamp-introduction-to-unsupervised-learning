...
* **Curse of intrinsic dimensionality (sensitive to intrinsic dimension):** Intrinsic Dimension is the number of variables needed to generate a good approximation of the signal. t-SNE performs poorly if the high dimensional data actually has high intrinsic dimensionality.
* **Non-convexity of the t-SNE cost function:** we must choose several optimization parameters to get a good result from the t-SNE algorithm.
